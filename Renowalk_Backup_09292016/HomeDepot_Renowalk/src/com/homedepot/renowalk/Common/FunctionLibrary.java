...
//	public static PrepareExcelReport objEx=new PrepareExcelReport();
	...
	public void fnCheckHandleModalDiv() {
		try {
			boolean isModalVisible = false;
			isModalVisible = ElementFound("//div[@class='modal-content']");
			if (isModalVisible) {
				driver.findElement(By.xpath("//button[@class='btn btn-default uibDialogOk ng-binding ng-scope' and text()='Yes']")).click();
			}
		} catch (Exception e) {
			TestDriver.log.info("Error handling Modal dialog div");
		}
	}
	...
	public int CheckByClass(String sLocator, String sReportText, boolean bReportFlag) {
		int result = 0;
		String sText ="";
		try {
			List<WebElement> eleList = driver.findElements(By.className(sLocator));
			result = eleList.size();
			if(bReportFlag==true)
			{
				if(result==1)
...
//				}
	 ...
	 public void fnIHLoadingPageWait() throws Exception

	 {
		try
		{
			WebDriverWait wait = new WebDriverWait(driver,120);
			CheckByClass(RenowalkModules.Property_IH_loadingimage_Class,"'IH Loading...' icon",true);
			wait.until(ExpectedConditions.invisibilityOfElementLocated(By.className(RenowalkModules.Property_IH_loadingmessage_Class)));	 //Wait for loading message to disappear
	 ...
	 {  ///sActionType = Add, Update ,Delete, OfficeCode
		...
		try
		{
			...
			if(ElementFound(RenowalkModules.Common_AddModule_TextDialogBox) )
			{
					...
					else if(sActionType.equalsIgnoreCase("LeaveCurrentPage"))
					{
						if(sText.equalsIgnoreCase(AppMessages.Common_Leavecurrentpage_DialogBox_msg))
						{
							bFlag = true;
						}
...
/*			if(sColumnName.equalsIgnoreCase("Last Activity"))  // On clicking last activity sort, it first get in descending order 
		...
		try
		{
			...
			{	
				...
				for(int iCol=0;iCol<arrTableColumns.size();iCol++)
				{
					...
					List<WebElement> arrActiveInactive;	
					...
					else if(sModuleName.equalsIgnoreCase("Corporations") || sModuleName.equalsIgnoreCase("Regions") || sModuleName.equalsIgnoreCase("Product Tiers")) 
					{
						switch (iCol) 
						{
							case 0: 
					...
					else if(sModuleName.equalsIgnoreCase("Users")) 
					{
						switch (iCol) 
						{
							case 0: 
							...
							case 1: case 2: case 3: case 5:
							        ...
							        break;
							case 4:
								//arrActiveInactive = arrTableColumns.get(iCol).findElements(By.xpath("./div/span"));
								arrActiveInactive = arrTableColumns.get(iCol).findElements(By.xpath("./div[not(contains(@class,'ng-hide'))]/span"));
								String sActiveInactive  = arrActiveInactive.get(0).getAttribute("title");
								
								//System.out.println(sEdit+sDelete);
								sColValue= sActiveInactive;
						        break;
							case 6: 
									arrEditDelete = arrTableColumns.get(iCol).findElements(By.xpath("./div/span"));
									String sEdit  = arrEditDelete.get(0).getAttribute("title");
	 ...
	 public int fnGetTableRowsCount(String sLocator) throws Exception 
		...
		public void fnVerifyComboBoxValue(String SelectValue, String TextValue) throws Exception
		{
		  ...
		  */
		 public void fnVerifySelectedCheckBoxByXPath(String sLocator, String checkBoxName) throws Exception 
		 {
			 try {
		  ...
		  * Method to verify if the check box is selected
		  */
		 public void verifyCheckBoxIsNotCheckedById(String sLocator, String checkBoxName) throws Exception 
		 {
			 try {				 
				 WebElement element = driver.findElement(By.id(sLocator));
				 HighlightElement(element);
				 
				 if(!element.isSelected())
				 {
					 obj.repAddData( "Verify "+checkBoxName+" is not checked", checkBoxName+" is not Checked", checkBoxName+" is not checked", "Pass");
				 }
				 else
				 {
					 obj.repAddData( "Verify "+checkBoxName+" is checked", checkBoxName+" is not checked", checkBoxName+" is checked", "Fail");
				 }

			 } catch (Exception e) {
				 System.out.println("verifyCheckBoxIsNotCheckedId --------------Failed");  
				 TestDriver.log.error("verifyCheckBoxIsNotCheckedId failed",e);
				 throw(e);
			 }
		 }
		 public void verifyCheckBoxIsNotCheckedByXpath(String sLocator, String checkBoxName) throws Exception 
		 {
			 try {				 
		 ...
		 public void verifyCheckBoxIsCheckedByXpath(String sLocator, String checkBoxName) throws Exception 
		 {
			 try {				 
				 WebElement element = driver.findElement(By.xpath(sLocator));
				 HighlightElement(element);
				 ...
				 if(element.isSelected())
				 {
					 obj.repAddData( "Verify "+checkBoxName+" is checked", checkBoxName+" is Checked", checkBoxName+" is checked", "Pass");
				 }
				 else
				 {
					 obj.repAddData( "Verify "+checkBoxName+" is checked", checkBoxName+" is not checked", checkBoxName+" is checked", "Fail");
		 ...
		 public void fnCheckSelectedCheckBoxByXPath(String sLocator, String checkBoxName) throws Exception 
		 {
			 try {
				 ...
				 System.out.println("fnCheckSelectedCheckBoxByXPath --------------Failed");  
				 TestDriver.log.error("fnCheckSelectedCheckBoxByXPath failed",e);
				 throw(e);
			 }
		 }
		 public String fnAddAndVerifyUser(String sUserType) throws Exception 
		 {
			 //sUserType=Corporate admin, Office admin, Office reviewer
			 try {
				 int iRandomNumber=fnRandomNum(11111,99999);	
					ClickByXpath(RenowalkModules.Users_GenOffice_LinkOfficesOption_xp, "Users Link", true);
					fnLoadingPageWait();
			...
			//		String sAutoTestEmail = mTestPhaseData.get(TestDriver.iTC_ID).get("Email").toString().trim();
					SendKeyByXpath(RenowalkModules.USERS_EMAIL_INPUT_XP, "AutoTestEmail_"+iRandomNumber+"@test.com","AutoTestEmail input");
					fnLoadingPageWait();
			//	String sAutoTestFN = mTestPhaseData.get(TestDriver.iTC_ID).get("Firstname").toString().trim();
					SendKeyByXpath(RenowalkModules.USERS_FIRSTNAME_INPUT_XP, "AutoTestFN_"+iRandomNumber, "AutoTest First name");
					fnLoadingPageWait();
			//		String sAutoTestLN = mTestPhaseData.get(TestDriver.iTC_ID).get("Lastname").toString().trim();		
					SendKeyByXpath(RenowalkModules.USERS_LASTNAME_INPUT_XP, "AutoTestLN_"+iRandomNumber, "AutoTest last name");
					
					if(sUserType.equalsIgnoreCase("Corporate Admin")) {
						fnSelectFromComboBoxXpath(RenowalkModules.USERS_SECURITYROLE_COMBOBOX_XP,"Corporate Admin");
						fnSelectFromComboBoxXpath(RenowalkModules.USERS_CORPORATION_COMBOBOX_XP, "AutoTestCorp");
					}
					else if(sUserType.equalsIgnoreCase("Office Admin")) {
					...
					else if(sUserType.equalsIgnoreCase("Office Reviewer")) {
				...
				//	String sPhoneNumber = mTestPhaseData.get(TestDriver.iTC_ID).get("PhoneNumber").toString().trim();					
					SendKeyByXpath(RenowalkModules.USERS_PHONENUMBER_INPUT_XP, "12345"+iRandomNumber, "Phone number");
					verifyCheckBoxIsCheckedByXpath(RenowalkModules.USERS_ACTIVECHECKBOX_XP, "Active check box");
					ClickById(RenowalkModules.USERS_SAVE_BTN_ID, "Save button", true);
					fnLoadingPageWait();
					...
					mTableData.size();
					String actualUserName="AutoTestEmail_"+iRandomNumber+"@test.com";
					boolean verifyRecord = false;
					for(int i=1;i<=mTableData.size();i++)
					{
						String sUserName = mTableData.get(i).get(1).toString();	
						
						if(sUserName.equalsIgnoreCase(actualUserName))
						{
							verifyRecord = true;
							break;
						}
					...
					if (verifyRecord == true) {
						obj.repAddData("Verify user based on email address", "Verified user based on email address", "Verified user",
								"Pass");
					} else {
						obj.repAddData("Verify user based on email address", "Unable to verify user based on email address", "User not Verified",
								"Fail");
					}		
					 return actualUserName;

			 } catch (Exception e) {
				 System.out.println("fnAddAndVerifyUser --------------Failed");  
				 TestDriver.log.error("fnAddAndVerifyUser failed",e);
				 throw(e);
			 }
			
		 }		 
		 
		 public void fnVerifyUser(String sUserName) throws Exception 
		 {
			 //sUserName= string returned from fnAddAndVerifyUser or hardcoded username
			 try {
					...
					for(int i=1;i<=mTableData.size();i++)
					{
						String tableUserName = mTableData.get(i).get(1).toString();	
						
						if(tableUserName.equalsIgnoreCase(sUserName))
						{
							verifyRecord = true;
							break;
						}
					...
					if (verifyRecord == true) {
						obj.repAddData("Verify user based on email address", "Verified user based on email address", "Verified the user",
								"Pass");
					} else {
						obj.repAddData("Verify user based on email address", "Unable to verify user based on email address", "User not Verified",
								"Fail");
				 ...
				 System.out.println("fnVerifyUser --------------Failed");  
				 TestDriver.log.error("fnVerifyUser failed",e);
				 throw(e);
			 }
			
		 }		 
		 
		 public void fnVerifyDeletedUser(String sUserName) throws Exception 
		 {
			 //sUserName= string returned from fnAddAndVerifyUser or hardcoded username
			 try {
					...
					if (verifyRecord == true) {
						...
						obj.repAddData("Verify user based on email address", "Unable to verify user as the user got deleted", "User not Verified as the user got deleted",
								"Pass");
					}		
					 

			 } catch (Exception e) {
				 System.out.println("fnVerifyDeletedUser --------------Failed");  
				 TestDriver.log.error("fnVerifyDeletedUser failed",e);
				 throw(e);
			 }
			
		 }		 
		 public void fnWaitForElementByXPath(String xpath) throws Exception {
				try {
					WebDriverWait wait = new WebDriverWait(driver, 60);
					wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(xpath))); // Wait for the element to appear																				
				} catch (Exception e) {
					System.out.println("fnWaitForElement By Xpath --------------Failed");
					TestDriver.log.error("fnWaitForElementByXpath failed",e);
					 throw(e);					
				}
			}
		 public void fnWaitForElementByID(String id) throws Exception {
				try {
					WebDriverWait wait = new WebDriverWait(driver, 60);
					wait.until(ExpectedConditions.presenceOfElementLocated(By.id(id))); // Wait for the element to appear																				
				} catch (Exception e) {
					System.out.println("fnWaitForElement By ID --------------Failed");
					TestDriver.log.error("fnWaitForElementByID failed",e);
					 throw(e);					
				}
			}
